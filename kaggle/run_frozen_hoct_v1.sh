#!/usr/bin/env bash
set -euo pipefail
mode=${1:?Usage: run_frozen_hoct_v1.sh smoke|full RUN_NAME [SMOKE_DIR]}
...
extra=()
if [ "$mode" = full ]; then extra=(--smoke-dir "${3:?Full mode requires completed v1 smoke}"); fi
...
python -m unittest discover -s tests -p test_hoct_v1_preservation.py -v
timeout "$budget" python -u kaggle/run_hoct_v1_consensus.py run \
