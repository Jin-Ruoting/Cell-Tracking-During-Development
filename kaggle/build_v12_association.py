#!/usr/bin/env python3
"""Package the frozen E040 GPU experiment from pushed local sources only."""
...
def build(args):
    ...
    if revision != subprocess.check_output(["git", "rev-parse", "@{upstream}"], cwd=root, text=True).strip():
        raise ValueError("Push local source before packaging E040")
    files = packaging.source_closure(root, ["kaggle/run_v12_association.py", "tests/test_v12_edge_swap.py", "tests/test_v12_association.py"])
    for name, source in files.items():
        if source != subprocess.check_output(["git", "show", f"HEAD:{name}"], cwd=root).decode():
            raise ValueError("Uncommitted E040 source: " + name)
    protocol = packaging.protocol_digest(files)
    # Parse the fixed config without importing NumPy or executing any model locally.
    import ast
    tree = ast.parse(files["kaggle/v12_edge_swap.py"])
    config_node = next(n.value for n in tree.body if isinstance(n, ast.Assign) and any(isinstance(t, ast.Name) and t.id == "CONFIG" for t in n.targets))
    import math
    allowed = {"math": math}
    config = eval(compile(ast.Expression(config_node), "fixed-source-config", "eval"), {"__builtins__": {}}, allowed)
    receipt = json.loads(args.cpu_receipt.read_text())
    if (receipt.get("passed") is not True or receipt.get("experiment") != "E040"
            or receipt.get("mode") != "fixed_node_cpu_technical" or receipt.get("ground_truth_accessed") is not False
            or receipt.get("strict_model_load") is not True or receipt.get("config") != config
            ...
            or receipt["source_manifest"]["files"]["v12_edge_swap.py"] != packaging.sha256(files["kaggle/v12_edge_swap.py"].encode())):
        raise ValueError("Completed CPU check does not validate the fixed node/swap policy")
    if args.mode == "smoke":
        ...
        if args.full_control_dir is None or args.smoke_receipt is None:
            raise ValueError("Full E040 requires completed S213 control and same-protocol GPU smoke")
        ...
        if (smoke.get("experiment") != "E040" or smoke.get("mode") != "smoke"
                ...
                or smoke["official_node_and_division_components_unchanged"] is not True):
    ...
    manifest = {"experiment": "E040", "git_commit": revision, "mode": args.mode, "datasets": names,
...
bundle = Path("/kaggle/working/e040-bundle")
...
logs = Path("/kaggle/working/logs/e040-launch")
...
runner = [sys.executable, str(bundle / "kaggle/run_v12_association.py"), "--bundle", str(bundle), "--phase"]
run("semantic-tests", [sys.executable, "-m", "unittest", "discover", "-s", "tests", "-p", "test_v12_*.py", "-v"])
...
print(Path("/kaggle/working/logs/e040-association/run_summary.md").read_text())
'''
    compile(code, "e040-gpu-bootstrap", "exec")
    metadata = {"id": f"{args.owner}/biohub-e040-fixed-node-{args.mode}",
                "title": "Biohub E040 Fixed Node " + args.mode.title(), "code_file": "e040.ipynb",
    ...
    notebook = {"nbformat": 4, "nbformat_minor": 5,
                ...
                "cells": [{"cell_type": "markdown", "metadata": {}, "id": "protocol", "source":
                           "# E040 frozen-node association\n\nPrivate paired development; frozen E029 control reused. "
                           "Nodes, coordinates, degrees and division edges remain fixed. No parameter search or submission.\n\n"
    ...
    for name, value in (("kernel-metadata.json", metadata), ("e040.ipynb", notebook), ("bundle_manifest.json", manifest)):
