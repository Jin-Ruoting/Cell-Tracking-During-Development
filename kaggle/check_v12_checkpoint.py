...
Local mode packages committed sources only. Runtime mode uses synthetic tensors,
not competition images/labels, and never produces a tracking submission.
"""
...
subprocess.run([sys.executable, str(root / "check_v12_checkpoint.py"), "run", "--source-commit", {commit!r}], env=env, check=True, timeout=600)
'''
    notebook = {"nbformat": 4, "nbformat_minor": 5,
                "metadata": {"kernelspec": {"display_name": "Python 3", "language": "python", "name": "python3"}},
                "cells": [{"cell_type": "markdown", "metadata": {}, "id": "scope", "source":
                           "# V12 CPU compatibility\n\nSynthetic tensor audit only. No images, labels, quality score or submission. "
...
def run(commit: str):
    ...
    try:
        ...
        (WORK / "run_summary.md").write_text("# V12 CPU compatibility audit\n\n```json\n" + json.dumps(report, indent=2) +
                                            "\n```\n\nArchitecture check only; no tracking-quality evidence.\n")
