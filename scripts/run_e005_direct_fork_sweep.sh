...
RUN_ID="${BIOHUB_E005_FORK_RUN_ID:-20260724}"
...
OUTPUT_ROOT="${BIOHUB_E005_FORK_DIR:-/tmp/biohub_e005_direct_fork_sweep_${RUN_ID}}"
RULES="${BIOHUB_E005_FORK_RULES:-${REPO_DIR}/configs/e005_division_rules_v1.json}"
INFERENCE_DONE="${LOG_DIR}/s041_e005_division_calibration_inference_${INFERENCE_RUN_ID}.done"
LOG_PATH="${LOG_DIR}/s044_e005_direct_fork_sweep_${RUN_ID}.log"
REPORT_PATH="${LOG_DIR}/s044_e005_direct_fork_sweep_${RUN_ID}.json"
DONE_PATH="${LOG_DIR}/s044_e005_direct_fork_sweep_${RUN_ID}.done"
...
score_candidate() {
  ...
  bash scripts/run_official_geff_scorer.sh \
    "$prediction_dir" \
    "$DATA_ROOT/train" 2>&1 | tee "$LOG_DIR/s044_e005_score_${label}_${RUN_ID}.log"
}

score_candidate baseline "$INFERENCE_DIR/baseline"
while IFS= read -r rule; do
  score_candidate "$rule" "$OUTPUT_ROOT/$rule"
done < <(jq -r '.[].name' "$RULES")
